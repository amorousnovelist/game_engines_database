...
(c) Компактный просмотр всех записей\n\
    с отображением макс. полигонов\n\
...
Введите 'y', если вы хотите, введите 'n' в ином случае: ");
		...
		case 'c': {
			...
			printf("=============================================================================================================================\n");
			printf("| № записи | макс.полигонов |\n");
			for (int i = 0; i < records_count; i++)
				compact_print_record(p_database, i);
			printf("=============================================================================================================================\n");
			break;
		}
		/*
		...
		* 
		* Разбить на 2 команды поиска:
		* - по названии технологии технологии рендеринга (-)
		...
		*/
		case 'r': {
			...
			if (records_count < 3) {
				printf("Ошибка поиска записей: база данных содержит недостаточно запискей для поиска.\nИнициализируйте базу данных хотя бы ещё %d записью(-ями).\n", 3 - records_count);
				break;
			}
			...
			*/
			...
			/*Ожидается порядковый номер записи (не индекс) для удобства пользователя*/
			printf("Введите начальное значение интервала поиска (от %d до %d включительно): ", 1, records_count - 2);
			scanf(" %d", &begin); 
		...
		case 's': {
			...
			*/
			//bubble_sort(p_database, records_count);
		...
		case 't': {
			...
			*/
			...
			polygons = (rand() % 99);
			unsigned platforms_count = 1 + (rand() % 100);
